...
def crop_and_save(images_path, masks_path, new_images_path, new_masks_path, img_width, img_height):
    ...
    for image_file in tqdm(files, total = len(files)):
        ...
        # Images came labeled as sat or mask, need to remove those labels in the file names
        image_file = image_file.split('_', 1)[0]

        image_path = images_path + image_file + '_sat.jpg' 
        image = cv2.imread(image_path)
        
        mask_path = masks_path + image_file + '_mask.png'
        ...
        for r in range(0, image.shape[0], img_height):
            for c in range(0, image.shape[1], img_width):
                ...
                new_image_path = new_images_path + str(counter) + '_' + image_file + '.jpg' 
                new_mask_path = new_masks_path + str(counter) + '_' + image_file + '.png'
...
if __name__ == "__main__":
    root_data_path = "D:/Users/Johnathan/Google Drive/Grad School/Penn_MUSA/Spring2022/650_RemoteSensing/FinalPrj/data/train/"
    test_to_train_ratio = 0.3 
    img_width = img_height = 153
    num_channels = 3
    ...
    new_images_path = root_data_path + "ImagesCropped/"
    new_masks_path = root_data_path + "MasksCropped/"
